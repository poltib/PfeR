...
(function() {
  var NUMBER_DOT_FIVENUMBERS, NUMBER_DOT_TWONUMBERS, SAMPLES, calc_bounds, chart, display_on_map, drawPath, elevationReqActive, elevationService, elevations, gm_init, gm_service, growPath, load_track, newRoute, path, plotElevation, poly, poly_init;
  ...
  growPath = function(origin, destination) {
  ...
  drawPath = function(path) {
    ...
    return elevationService.getElevationAlongPath(pathRequest, plotElevation);
  };
  ...
  plotElevation = function(results, status) {
    ...
    for (_i = 0, _len = results.length; _i < _len; _i += 1) {
      i = results[_i];
  ...
  $(function() {
    var clear, close, dist, distanceInput, elevation_chart, getZipCode, jsInput, locationInput, map, reset, save, upLi;
    ...
    if (save != null) {
      ...
      google.maps.event.addListener(map, 'click', function(evt) {
        if (path.getLength() === 0) {
          ...
          return growPath(path.getAt(path.getLength() - 1), evt.latLng);
        }
      });
      reset.addEventListener('click', function(evt) {
        ...
        if (mousemarker !== null) {
          return mousemarker.setMap(null);
        }
      });
      close.addEventListener('click', function(evt) {
        if (path.getLength() !== 0) {
          return growPath(path.getAt(path.getLength() - 1), path.getAt(0));
        }
      });
      save.addEventListener('click', function(evt) {
        ...
        for (_i = 0, _len = elevations.length; _i < _len; _i += 1) {
          i = elevations[_i];
          newRoute[_i] = elevations[_i].location.A.toString().replace(NUMBER_DOT_FIVENUMBERS, '$1$2') + ',' + elevations[_i].location.k.toString().replace(NUMBER_DOT_FIVENUMBERS, '$1$2');
        }
      ...
      return getZipCode = function(lat, lon) {
        var url;
        ...
        return $.ajax({
          ...
          _ref = data.results[0].address_components;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            c = _ref[_i];
            if (c.types[0] === 'postal_code') {
              _results.push(locationInput.value = c.short_name);
            } else {
